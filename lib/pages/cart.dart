...
class _CartPageState extends State<CartPage> {
  ...
  Future<void> loadCart() async {
    ...
    if (cartJson != null && cartJson.isNotEmpty) {
      ...
      setState(() {
        cartProducts = decoded.map((e) => Map<String, dynamic>.from(e)).toList();
  ...
  Future<void> saveCart() async {
    ...
    if (cartProducts.isEmpty) {
      ...
      await prefs.setString('cart_products', jsonEncode(cartProducts));
    }
  }

  void removeProduct(int index) async {
    if (index < cartProducts.length && index < selectedItems.length) {
      setState(() {
  ...
  double getTotalPrice() {
    ...
    for (int i = 0; i < cartProducts.length; i++) {
      if (i < selectedItems.length && selectedItems[i]) {
        double price = double.tryParse(cartProducts[i]['product_price'].toString()) ?? 0.0;
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      if (product['image_path'] != null) {
                        try {
                          imageExists = File(product['image_path']).existsSync();
                        } catch (e) {
                          imageExists = false;
                        }
                      }
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            Checkbox(
                              value: selectedItems.length > index && selectedItems[index],
